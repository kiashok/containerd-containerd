...
type Store struct {
	lock sync.RWMutex
	// refCache is a containerd image refKey to image id key.
...
type store struct {
	...
	// Images will not be removed from cri store until there are no
	// more images references this image.
	// image digest -> imageIDKey referencing the image
