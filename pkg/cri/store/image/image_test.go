...
func TestInternalStore(t *testing.T) {
	...
	for _, v := range images {
		...
		refCacheKey := RefCacheKey{Ref: oldRef, RuntimeHandler: ""}
		s.delete(truncID, refCacheKey)
		got, err = s.get(truncID, "")
		assert.NoError(err)
		assert.Equal([]string{newRef}, got.References)

		t.Logf("should be able to delete image")
		refCacheKey = RefCacheKey{Ref: newRef, RuntimeHandler: ""}
		s.delete(truncID, refCacheKey)
		got, err = s.get(truncID, "")
...
func TestInternalStorePinnedImage(t *testing.T) {
	...
	assert.False(s.isPinned(image.Key.ID, ref1))
	...
	assert.False(s.isPinned(image.Key.ID, ref1))
	assert.True(s.isPinned(image.Key.ID, ref2))

	t.Logf("pin unpinned image ref, image should be pinned, all refs should be pinned")
	assert.NoError(s.pin(image.Key.ID, ref1))
	i, err = s.get(image.Key.ID)
	assert.NoError(err)
	assert.True(i.Pinned)
	assert.True(s.isPinned(image.Key.ID, ref1))
	assert.True(s.isPinned(image.Key.ID, ref2))

	t.Logf("unpin one of image refs, image should be pinned")
	assert.NoError(s.unpin(image.Key.ID, ref2))
	i, err = s.get(image.Key.ID)
	assert.NoError(err)
	assert.True(i.Pinned)
	assert.True(s.isPinned(image.Key.ID, ref1))
	assert.False(s.isPinned(image.Key.ID, ref2))

	t.Logf("unpin the remaining one image ref, image should be unpinned")
	assert.NoError(s.unpin(image.Key.ID, ref1))
	i, err = s.get(image.Key.ID)
	assert.NoError(err)
	assert.False(i.Pinned)
	assert.False(s.isPinned(image.Key.ID, ref1))
