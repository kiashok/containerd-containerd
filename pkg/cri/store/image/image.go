...
func NewStore(img images.Store, provider InfoProvider, platform platforms.MatchComparer) *Store {
	//, platformMatcherMap map[string]platforms.MatchComparer) *Store {
	return &Store{
...
func (s *Store) Update(ctx context.Context, ref, platform string) error {
	s.lock.Lock()
	defer s.lock.Unlock()
	...
	// CRI image store
	platformImageLabelKey := fmt.Sprintf(ctrdlabels.PlatformsForImagePullFormat, ctrdlabels.PlatformsForImagePullPrefix, platform)
	if err == nil && i.Labels[platformImageLabelKey] == "" {
		refCacheKey := RefCacheKey{Ref: ref, PlatformForImage: platform}
	...
	return s.update(ref, platform, img)
}

// update updates the internal cache. img == nil means that
// the image does not exist in containerd.
func (s *Store) update(ref string, platform string, img *Image) error {
	refCacheKey := RefCacheKey{Ref: ref, PlatformForImage: platform}
	...
	if oldExist {
		if oldImageIDKey.ID == img.Key.ID && oldImageIDKey.PlatformForImage == platform {
...
func (s *Store) getImage(ctx context.Context, i images.Image, platform string) (*Image, error) {
	...
	if platform != "" {
		...
		if err := json.Unmarshal([]byte(platform), &ocispecPlatform); err != nil {
			return nil, fmt.Errorf("unable to unmarshal runtimeHandlerPlatform %v", platform)
		}
		platformMatcher = platforms.Only(ocispecPlatform)
	}
...
func (s *Store) Resolve(ref, platform string) (string, error) {
	s.lock.RLock()
...
func (s *store) add(img Image) error {
	...
	if img.Pinned {
		...
		for _, references := range img.References {
			refCacheKeys = append(refCacheKeys, RefCacheKey{Ref: references, PlatformForImage: img.Key.PlatformForImage})
		}
...
func (s *store) pin(imageID string, refCacheKey RefCacheKey) error {
	...
	imageIDKey := ImageIDKey{ID: digest.String(), PlatformForImage: refCacheKey.PlatformForImage}
...
func (s *store) get(imageID, platform string) (Image, error) {
	s.lock.RLock()
	defer s.lock.RUnlock()
	digest, err := s.digestSet.Lookup(imageID)
	if err != nil {
	...
	imageIDKey := ImageIDKey{ID: digest.String(), PlatformForImage: platform}
