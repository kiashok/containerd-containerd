...
func TestLocalResolve(t *testing.T) {
	...
	for _, ref := range []string{
		...
		img, err := c.LocalResolve(ref, "" /* use default runtime Handler if nothing is specified */)
		assert.NoError(t, err)
		assert.Equal(t, image, img)
	}
	img, err := c.LocalResolve("randomid", "" /* use default runtime Handler if nothing is specified */)
